'''
This is pytorch impplementation of PlantAIM(2H) from
...
# for precision, recall and f1 calculations
...
class CustomDatasetforPlantAIM(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Get plant and disease label of the image 
...
class Model_SL(nn.Module):
    def __init__(self,model_v, model_c, num_plant, num_disease):
    ...
    def forward(self, i1):
        ...
        self.h1d =  self.fc_d(self.f3[:, 0] + self.f1[:, 0])  
        return (self.h1p, self.h1d)
...
def main():    
    ...
    data_path1 = 'D:/DPD_dataset/PV/plantvillage_ori/plantvillage_(mix)'
    ...
    train_csv_path = './csv_plantaim/PV_train_3L_10s.csv' 
    test_csv_path1 = './csv_plantaim/PV_test_seen_3L.csv' 
    test_csv_path2 = './csv_plantaim/PV_test_limited_3L.csv' 
    test_csv_path3 = './csv_plantaim/IPM_test_3L.csv' 
    test_csv_path4 = './csv_plantaim/Bing_test_3L.csv' 
    test_csv_path5 = './csv_plantaim/Plantdoc_test_3L.csv' 
    
    # Dataset declarations
    train_dataset = CustomDatasetforPlantAIM(train_csv_path, transforms = train_transforms, data_path = data_path1)
    test_dataset1 = CustomDatasetforPlantAIM(test_csv_path1, transforms = valid_transforms, data_path = data_path1)
    test_dataset2 = CustomDatasetforPlantAIM(test_csv_path2, transforms = valid_transforms, data_path = data_path1)
    test_dataset3 = CustomDatasetforPlantAIM(test_csv_path3, transforms = valid_transforms, data_path = data_path2)
    test_dataset4 = CustomDatasetforPlantAIM(test_csv_path4, transforms = valid_transforms, data_path = data_path2)
    test_dataset5 = CustomDatasetforPlantAIM(test_csv_path5, transforms = valid_transforms, data_path = data_path3)
    ...
    test_loader3 = DataLoader(test_dataset3, batch_size=batch_size, num_workers=num_workers, shuffle=False, drop_last = False)
    test_loader4 = DataLoader(test_dataset4, batch_size=batch_size, num_workers=num_workers, shuffle=False, drop_last = False)
    test_loader5 = DataLoader(test_dataset5, batch_size=batch_size, num_workers=num_workers, shuffle=False, drop_last = False)
    ...
    base_resnet = models.resnet152(pretrained=pretrained)
    model_c = ResNetWithFeatures(base_resnet)
    model_c.to(device)
    
    # Tensorboard for progress monitoring
    tb = SummaryWriter()
    
    model_sl = Model_SL(model_v, model_c, num_plant, num_disease)
    model_sl.to(device)
    # print(model_sl)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, labels_p, labels_d) in enumerate(tqdm(train_loader)):        
            images, labels_p, labels_d  = images.to(device), labels_p.to(device), labels_d.to(device)
    
            # Obtain the output for plant and disease prediction
            output_p, output_d = model_sl(images)
            
            # Calculate loss from plant and disease prediction
            loss_p = error(output_p, labels_p)
            loss_d = error(output_d, labels_d)
            loss_t = loss_p + loss_d
            ...
            d_predictions = torch.max(output_d,1)[1].to(device)
            
            # Calculate accuracy
            correct_train_p += (p_predictions == labels_p).sum()
            correct_train_d += (d_predictions == labels_d).sum()
    
            # Post calculation for plant disease label
            for x in range(len(labels_p)):
                if (p_predictions[x] == labels_p[x]):
                    if (d_predictions[x] == labels_d[x]):
                        correct_train_o_combine += 1 
            ...
            # Tensorboard record        
            tb.add_scalar("Total SL loss for Plant", total_loss_p, epoch)
            tb.add_scalar("Total SL loss for Disease", total_loss_d, epoch)
        ...
        a_p = total_loss_p / ((len(train_dataset)//batch_size) + (len(train_dataset) % batch_size > 0))
        a_d = total_loss_d / ((len(train_dataset)//batch_size) + (len(train_dataset) % batch_size > 0))
        tb.add_scalar("Loss per epoch", (a_p+a_d), epoch)
        print('SL loss for Plant: {0}'.format(a_p))
        print('SL loss for Disease: {0}'.format(a_d))
        print('Total Loss: {0}'.format(a_p+a_d))
        ...
        with torch.no_grad():
            ...
            for images, labels_p, labels_d in tqdm(test_loader1):
                ...
                d_prediction = torch.max(output_d,1)[1].to(device)
        
                # Calculate accuracy
                correct_p += (p_prediction == labels_p).sum()
                correct_d += (d_prediction == labels_d).sum()
                
                # Post calculation for plant disease label
                for x in range(len(labels_p)):
                    if (p_prediction[x] == labels_p[x]):
                        if (d_prediction[x] == labels_d[x]):
        ...
        # for precision, recall and f1 calculations
            ...
            # Append predictions for both heads
            # all_preds_head1.append(p_prediction)
            # all_preds_head2.append(d_prediction)
            # all_labels1.append(labels_p)
            # all_labels2.append(labels_d)        
    
        # Concatenate predictions and labels
        # all_preds_head1 = torch.cat(all_preds_head1)
        # all_preds_head2 = torch.cat(all_preds_head2)
        # all_labels1 = torch.cat(all_labels1)
        # all_labels2 = torch.cat(all_labels2)
        
        # # Calculate metrics for both classifier heads
        # precision_head1, recall_head1, f1_head1 = calculate_metrics(all_preds_head1, all_labels1)
        # precision_head2, recall_head2, f1_head2 = calculate_metrics(all_preds_head2, all_labels2)
        # print("")
        # print(f'Head 1 - Precision: {precision_head1:.4f}, Recall: {recall_head1:.4f}, F1 Score: {f1_head1:.4f}')
        # print(f'Head 2 - Precision: {precision_head2:.4f}, Recall: {recall_head2:.4f}, F1 Score: {f1_head2:.4f}')
        
        # Total accuracy calculations
        accuracy_p = correct_p * 100 / total
        accuracy_d = correct_d * 100 / total
        ...
        # Tensorboard record 
        tb.add_scalar("Testing Loss per epoch", (a_p+a_d), epoch)
        tb.close()
        print("")
        print('Total testing Loss: {0}'.format(a_p+a_d))    
        print("")
        print("Testing acc for seen Plant : {:.4f}".format(accuracy_p))
        print("")
        print("Testing acc for seen Disease : {:.4f}".format(accuracy_d))
        print("")    
        print("Testing acc for total seen PD : {:.4f}".format(accuracy_o))
        print("")
        ...
        accuracy_p_1 = correct_p * 100 / total
        accuracy_d_1 = correct_d * 100 / total
        accuracy_o = correct_test_o_combine  * 100 / total
           
        print("")
        print("Testing acc for FS Plant : {:.4f}".format(accuracy_p_1))
        print("")
        print("Testing acc for FS Disease : {:.4f}".format(accuracy_d_1))
        print("")
        print("Testing acc for total FS PD : {:.4f}".format(accuracy_o))
        print("")
        ...
        # for images, labels_p, labels_d in tqdm(test_loader3):
        #     images, labels_p, labels_d = images.to(device), labels_p.to(device), labels_d.to(device)
        ...
        # print("")
        # print("Testing acc for IPM Plant : {:.4f}".format(accuracy_p_1))
        # print("")
        # print("Testing acc for IPM Disease : {:.4f}".format(accuracy_d_1))
        # print("")
        # print("Testing acc for total IPM PD : {:.4f}".format(accuracy_o))
        # print("")
        ...
        # for images, labels_p, labels_d in tqdm(test_loader4):
        #     images, labels_p, labels_d = images.to(device), labels_p.to(device), labels_d.to(device)
        ...
        # print("")
        # print("Testing acc for Bing Plant : {:.4f}".format(accuracy_p_1))
        # print("")
        # print("Testing acc for Bing Disease : {:.4f}".format(accuracy_d_1))
        # print("")
        # print("Testing acc for total Bing PD : {:.4f}".format(accuracy_o))
        # print("")
        ...
        # for images, labels_p, labels_d in tqdm(test_loader5):
        #     images, labels_p, labels_d = images.to(device), labels_p.to(device), labels_d.to(device)
        ...
        # print("")
        # print("Testing acc for PlantDoc Plant : {:.4f}".format(accuracy_p_1))
        # print("")
        # print("Testing acc for PlantDoc Disease : {:.4f}".format(accuracy_d_1))
        # print("")
        # print("Testing acc for total PlantDoc Bing PD : {:.4f}".format(accuracy_o))
        # print("")
         
        
        if ((epoch+1) % 1 == 0):
            print("Saving Model")
            torch.save(model_sl.state_dict(), os.path.join(Save_model_path,'ViT+CNN_2H_{}e_{:.4f}_{:.4f}-plantaim2H_model.pth'
                                                                    .format(epoch+1,accuracy_train_d,accuracy_d)))
    
            print("Saving optimizer")
            torch.save(optimizer.state_dict(), os.path.join(Save_model_path,'ViT+CNN_2H_{}e_{:.4f}_{:.4f}-plantaim2H_opt.pth'
                                                                        .format(epoch+1,accuracy_train_d,accuracy_d)))
